...
-   **SWH_DM.DM_FL_MS** - Table containing the market segment allocated to each flight [from 1/01/2019]{style="text-decoration: underline;"}. Use the flight ID field ('FLT_UID') and 'LOBT' field (partition key) to link to other flight tables (e.g. ARU_FLT.FLT, SWH_FCT.FAC_FLIGHT) and the field 'SK_FLT_TYPE_RULE_ID' to link to the table SWH_FCT.DIM_FLIGHT_TYPE_RULE to retrieve the name of the market segment.

-   SWH_FCT.FAC_FLIGHT - Flight table containing all information at flight level, including market segment information for flights [before 1/01/2019]{style="text-decoration: underline;"}.
...
**Important note**: The market segment for each flight used to be stored in the flight table SWH_FCT.FAC_FLIGHT, however since Summer 2023 this is no longer the case and, although the field 'SK_FLT_TYPE_RULE_ID' still exists and contains information for flights before 1/01/2019, for flights from this date, the market segment information in this table should not be used.
